...
typedef struct {
    unsigned int    start_address;
    unsigned int    end_address;
    PowerPC_func_node* funcs;
    unsigned long   adler32;
    unsigned char flags[1024];
} PowerPC_block;

#define MAX_JUMPS        4096
#define JUMP_TYPE_J      1
#define JUMP_TYPE_CALL   2
#define JUMP_TYPE_SPEC   4
typedef struct {
    unsigned int   src_pc;
...
#define EMIT_ISYNC() \
{PowerPC_instr ppc;GEN_ISYNC(ppc);set_next_dst(ppc);}
...
#define EMIT_RLDICR(ra,rs,sh,me) \
{PowerPC_instr ppc;GEN_RLDICR(ppc,ra,rs,sh,me,0);set_next_dst(ppc);}
#define EMIT_SLDI(ra,rs,sh) \
{PowerPC_instr ppc;GEN_RLDICR(ppc,ra,rs,sh,63-(sh),0);set_next_dst(ppc);}
