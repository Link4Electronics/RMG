...
/* mtspr 9, rs  =  mtctr rs
 * Correct split SPR encoding per Power ISA:
 *   PPC bits 6-10:  RS (source register)
 *   PPC bits 11-15: SPR[4:0] (low 5 bits of SPR number)
 *   PPC bits 16-20: SPR[9:5] (high 5 bits of SPR number)
 * For SPR=9 (CTR): SPR[4:0]=9, SPR[9:5]=0 */
#define GEN_MTCTR(ppc,rs) \
    { ppc = NEW_PPC_INSTR(); \
      PPC_SET_OPCODE(ppc, PPC_OPCODE_X); \
      PPC_SET_FUNC  (ppc, PPC_FUNC_MTSPR); \
      PPC_SET_RD(ppc, (rs));              /* RS at PPC bits 6-10 */ \
      ppc |= (9 & 0x1F) << 16;           /* SPR[4:0] = 9 at PPC bits 11-15 */ \
      ppc |= ((9 >> 5) & 0x1F) << 11;    /* SPR[9:5] = 0 at PPC bits 16-20 */ \
    }
...
#define GEN_MFCTR(ppc,rd) \
    { ppc = NEW_PPC_INSTR(); \
      ...
      PPC_SET_RD(ppc, (rd));              /* RT at PPC bits 6-10 */ \
      ppc |= (9 & 0x1F) << 16;           /* SPR[4:0] = 9 at PPC bits 11-15 */ \
      ppc |= ((9 >> 5) & 0x1F) << 11;    /* SPR[9:5] = 0 at PPC bits 16-20 */ \
    }
...
#define GEN_MTLR(ppc,rs) \
    { ppc = NEW_PPC_INSTR(); \
      ...
      PPC_SET_FUNC  (ppc, PPC_FUNC_MTSPR); \
      PPC_SET_RD(ppc, (rs));              /* RS at PPC bits 6-10 */ \
      ppc |= (8 & 0x1F) << 16;           /* SPR[4:0] = 8 at PPC bits 11-15 */ \
      ppc |= ((8 >> 5) & 0x1F) << 11;    /* SPR[9:5] = 0 at PPC bits 16-20 */ \
    }
...
#define GEN_FSQRTS(ppc,fd,fs) \
    { ppc = NEW_PPC_INSTR(); \
      PPC_SET_OPCODE(ppc, PPC_OPCODE_X); \
      PPC_SET_FUNC  (ppc, PPC_FUNC_FSQRTS); \
      PPC_SET_RD    (ppc, (fd)); \
      PPC_SET_RA    (ppc, (fs)); }
