...
/* Number of Count units between periodic returns to the C dispatcher.
 * genUpdateCount's BLELR compares Count against next_interrupt. For
 * pure-ALU compiled PPC loops (e.g., PIF boot ROM), no bypass call
 * occurs and the real next_interrupt may be far in the future
 * (SPECIAL_INT at 0x80000000). By clamping next_interrupt to
 * Count + PPCD_CHECK_INTERVAL before each dyna_run, the compiled code
 * returns periodically, allowing the dispatcher to service interrupts.
 * Chosen as a balance between responsiveness and overhead: ~5000 MIPS
 * instructions between checks. */
#define PPCD_CHECK_INTERVAL 10000u
...
void dynarec(unsigned int address){
    ...
    while(1){
        ...
        /* Clamp next_interrupt to ensure periodic return from compiled code.
         * Pure-ALU compiled PPC blocks (e.g., PIF boot ROM delay loop at
         * 0xA4000428) have no bypass calls and the real next_interrupt may
         * be far in the future (SPECIAL_INT at 0x80000000).  By setting
         * next_interrupt to min(real_next, Count + PPCD_CHECK_INTERVAL),
         * genUpdateCount's BLELR fires periodically and the dispatcher can
         * service genuine interrupts.  After dyna_run returns we restore
         * next_interrupt from the r4300 struct (which bypass handlers may
         * have updated) so the interrupt check sees the real value. */
        {
            uint32_t real_next = next_interupt;
            uint32_t clamped = Count + PPCD_CHECK_INTERVAL;
            if (clamped < real_next)
                next_interupt = clamped;
        }
        ...
        /* Restore real next_interrupt after periodic clamping */
        if (ppc_dynarec_r4300)
        ...
        if (dyna_canary[46]) {
            ...
            /* Sync CP0 Count to r4300 struct before C-coded handlers
             * access it. Compiled PPC code modifies Count (reg_cop0[9])
             * via stw, but hardware handlers (AI, PI, VI) use
             * r4300->cp0.regs[]. Without this sync, cp0_update_count()
             * in do_dma() reads stale Count and schedules interrupts
             * at wrong times (broken audio, etc.). */
            if (ppc_dynarec_r4300)
                ppc_dynarec_r4300->cp0.regs[CP0_COUNT_REG] = Count;
            ...
            case 2: /* dyna_check_cop1_unusable */
                ...
                break;
            case 3: /* decodeNInterpret */
                result = decodeNInterpret((MIPS_instr)dyna_canary[40],
                                          dyna_canary[41],
                                          (int)dyna_canary[42]);
                if (result == 0)
                    address = dyna_canary[41] + 4;
                else
                    address = result;
                break;
            default:
            ...
            /* Re-sync CP0 state that handlers may have changed.
             * AI/PI/VI handlers call cp0_update_count() (modifies
             * r4300->cp0.regs[CP0_COUNT_REG]) and add_interrupt_event()
             * (modifies r4300->cp0.next_interrupt). Bring these back
             * to dynarec globals so the interrupt check below works. */
            if (ppc_dynarec_r4300) {
                Count = ppc_dynarec_r4300->cp0.regs[CP0_COUNT_REG];
                next_interupt = ppc_dynarec_r4300->cp0.next_interrupt;
            }

            interp_addr = address;
            noCheckInterrupt = 0;
            /* Fall through to interrupt check instead of continuing,
             * so that newly-scheduled interrupts (e.g. AI_INT from
             * audio DMA) are serviced immediately. */
        }
...
unsigned int dyna_test(unsigned int value, unsigned int addr,
    ...
    dyna_canary[8] = 0xFE;  /* confirm function body is entered (slot 8 is C-code-only, not touched by compiled code or trampoline) */
    return 1;
}
...
void ppc_dynarec_cleanup(void) {
    ...
    for (i = 0; i < 0x100000; i++) {
        if (blocks[i]) {
            /* remove_outgoing_links frees the node tree internally via
             * MetaCache_Free — do NOT free(node) after it returns. */
            if (blocks[i]->funcs)
                remove_outgoing_links(&blocks[i]->funcs, NULL);
            blocks[i]->funcs = NULL;
            free(blocks[i]);
            blocks[i] = NULL;
        }
