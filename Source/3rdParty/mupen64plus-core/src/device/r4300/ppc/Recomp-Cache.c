...
/* Standard PPC two-loop D/I-cache coherency sequence.
 * 1. sync: drain store buffer so stores are visible to D-cache
 * 2. dcbf each cache line: write dirty data to memory and invalidate D-cache
 * 3. sync: ensure all dcbf complete
 * 4. icbi each cache line: invalidate I-cache
 * 5. sync: ensure all icbi complete
 * 6. isync: context synchronize so new instructions are fetched from memory */
static void FlushCacheRange(void* startaddr, unsigned int len)
{
    ...
    __asm__ __volatile__("sync" ::: "memory");
    start = (unsigned long)startaddr & ~31UL;
    for (; start < end; start += 32)
