...
void DLParser_Process(OSTask * pTask)
{
    ...
    try
    {
        ...
        while( gDlistStackPointer >= 0 )
        {
            ...
            { uint32 op = pgfx->words.w0 >> 24;
                if (op == 0xF0)
                    fprintf(stderr, "GBI_LOADTLUT: w0=0x%08X w1=0x%08X\n",
...
void DLParser_SetBlendColor(Gfx *gfx)
{
    ...
    gRDP.blendColor      = N64ColorToCOLOR_RGBA(gfx->setcolor.color); 
...
void DLParser_SetPrimColor(Gfx *gfx)
{
    ...
    gRDP.primitiveColor    = N64ColorToCOLOR_RGBA(gfx->setcolor.color);
...
void DLParser_SetEnvColor(Gfx *gfx)
{
    ...
    gRDP.envColor      = N64ColorToCOLOR_RGBA(gfx->setcolor.color); 
...
void LoadMatrix(uint32 addr)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++) 
        {
            int hi = *(short *)(g_pRDRAMu8 + ((addr+(i<<3)+(j<<1)     )^N64_XOR(2)));
            int lo = *(unsigned short *)(g_pRDRAMu8 + ((addr+(i<<3)+(j<<1) + 32)^N64_XOR(2)));
