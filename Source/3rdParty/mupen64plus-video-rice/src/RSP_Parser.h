...
{
#if __BIG_ENDIAN__
    // MSB-first: first declared field maps to bits 15-14 (MSB of uint16)
...
#else
    // LSB-first: first declared field maps to bits 1-0 (LSB of uint16)
    unsigned int    c2_m2b:2; // M (cycle 2)
    unsigned int    c1_m2b:2; // M (cycle 1)
    unsigned int    c2_m2a:2; // B (cycle 2)
    unsigned int    c1_m2a:2; // B (cycle 1)
...
typedef struct
{
    union
    {
        ...
        {
#if __BIG_ENDIAN__
            // Low bits (reversed field order for BE MSB-first packing)
            unsigned int        blender : 16;               // 31..16
            unsigned int        tex_edge : 1;               // 15
            unsigned int        force_bl : 1;               // 14
            unsigned int        alpha_cvg_sel : 1;          // 13
            unsigned int        cvg_x_alpha : 1;            // 12
            unsigned int        zmode : 2;                  // 11..10
            unsigned int        cvg_dst : 2;                // 9..8
            unsigned int        clr_on_cvg : 1;             // 7
            unsigned int        im_rd : 1;                  // 6
            unsigned int        z_upd : 1;                  // 5
            unsigned int        z_cmp : 1;                  // 4
            unsigned int        aa_en : 1;                  // 3
            unsigned int        depth_source : 1;           // 2
            unsigned int        alpha_compare : 2;          // 1..0

            // High bits (reversed field order for BE MSB-first packing)
            unsigned int        pad : 8;                    // 31..24
            unsigned int        atomic_prim : 1;            // 23
            unsigned int        reserved : 1;               // 22
            unsigned int        cycle_type : 2;             // 21..20
            unsigned int        text_persp : 1;             // 19
            unsigned int        text_detail : 1;            // 18
            unsigned int        text_sharpen : 1;           // 17
            unsigned int        text_lod : 1;               // 16
            unsigned int        text_tlut : 2;              // 15..14
            unsigned int        text_filt : 2;              // 13..12
            unsigned int        text_conv : 3;              // 11..9
            unsigned int        key_en : 1;                 // 8
            unsigned int        rgb_dither : 2;             // 7..6
            unsigned int        alpha_dither : 2;           // 5..4
            unsigned int        blend_mask : 4;             // 3..0
#else
            // Low bits
...
#define RDRAM_UHALF(addr)   (*(uint16 *)(((addr)^N64_XOR(2))+g_pRDRAMu8))
#define RDRAM_SHALF(addr)   (*(short *)(((addr)^N64_XOR(2))+g_pRDRAMu8))
#define RDRAM_UBYTE(addr)   (*(uint8 *)(((addr)^N64_XOR(3))+g_pRDRAMu8))
#define RDRAM_SBYTE(addr)   (*(s8 *)(((addr)^N64_XOR(3))+g_pRDRAMu8))
#define pRDRAM_UWORD(addr)  ((uint32 *)((addr)+g_pRDRAMu8))
#define pRDRAM_SWORD(addr)  ((s32 *)((addr)+g_pRDRAMu8))
#define pRDRAM_UHALF(addr)  ((uint16 *)(((addr)^N64_XOR(2))+g_pRDRAMu8))
#define pRDRAM_SHALF(addr)  ((short *)(((addr)^N64_XOR(2))+g_pRDRAMu8))
#define pRDRAM_UBYTE(addr)  ((uint8 *)(((addr)^N64_XOR(3))+g_pRDRAMu8))
#define pRDRAM_SBYTE(addr)  ((s8 *)(((addr)^N64_XOR(3))+g_pRDRAMu8))
