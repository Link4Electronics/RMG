...
// CI index 0 with black palette entry should be transparent (alpha=0)
// to match the N64 RDP behavior where CI index 0 has zero coverage.
// On LE, the GL format GL_BGRA + UNSIGNED_INT_8_8_8_8_REV accidentally
// reads alpha from the B byte (=0 for black) — we replicate that on BE.
#if defined(__BIG_ENDIAN__) || (defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__)
#define FORCE_CI_ZERO_ALPHA(c, idx, pal) do { \
    if ((idx) == 0 && ((pal) & 0xFFFE) == 0) (c) &= 0x00FFFFFF; \
} while(0)
#else
#define FORCE_CI_ZERO_ALPHA(c, idx, pal) ((void)0)
#endif
    ...
    if (tinfo.bSwapped)
    {
        for (uint32 y = 0; y <  tinfo.HeightToLoad; y++)
        {
            ...
            if (tinfo.WidthToLoad == 1)
            {
                ...
                uint8 b = pSrc[dwByteOffset ^ N64_XOR(nFiddle)];
                uint8 bhi = (b&0xf0)>>4;
                { uint16 pe = pPal[bhi^ N64_XOR(1)]; uint32 c = Convert555ToRGBA(pe); FORCE_CI_ZERO_ALPHA(c, bhi, pe); *pDst = c; }
                if( bIgnoreAlpha )
                {
                    *pDst |= 0xFF000000;
                }
            ...
            else for (uint32 x = 0; x < tinfo.WidthToLoad; x+=2)
            {
                ...
                { uint16 pe = pPal[bhi^ N64_XOR(1)]; uint32 c = Convert555ToRGBA(pe); FORCE_CI_ZERO_ALPHA(c, bhi, pe); pDst[0] = c; }
                { uint16 pe = pPal[blo^ N64_XOR(1)]; uint32 c = Convert555ToRGBA(pe); FORCE_CI_ZERO_ALPHA(c, blo, pe); pDst[1] = c; }
    ...
    if (tinfo.bSwapped)
    {
        for (uint32 y = 0; y < tinfo.HeightToLoad; y++)
        {
            ...
            for (uint32 x = 0; x < tinfo.WidthToLoad; x++)
            {
                ...
                { uint16 pe = pPal[b^ N64_XOR(1)]; uint32 c = Convert555ToRGBA(pe); FORCE_CI_ZERO_ALPHA(c, b, pe); *pDst++ = c; }
