...
static GLuint createShader( GLenum shaderType, const char* shaderSrc )
{
    ...
    if (status == GL_FALSE)
    {
        fprintf(stderr, "RICE: Compile shader failed:\n");
        fprintf(stderr, "RICE: Shader type: ");
        ...
        fprintf(stderr, "RICE: Info log:\n%s\n", pInfoLog);
        fprintf(stderr, "RICE: GLSL code:\n%s\n", shaderSrc);
...
void COGLColorCombiner::InitCombinerCycleCopy(void)
{
    ...
    if (logLen > 1) {
        ...
        if (strlen(log) > 0)
            fprintf(stderr, "RICE: InitCombinerCycleCopy log=%s\n", log);
        free(log);
    }
    (void)logLen;
    useProgram( program );
...
void COGLColorCombiner::InitCombinerCycleFill(void)
{
    ...
    float fr = ((gRDP.fillColor>>16)&0xFF)/255.0f;
    float fg = ((gRDP.fillColor>>8) &0xFF)/255.0f;
    float fb = ((gRDP.fillColor)    &0xFF)/255.0f;
    float fa = ((gRDP.fillColor>>24)&0xFF)/255.0f;
    glUniform4f( m_fillColorLoc, fr, fg, fb, fa);
    OPENGL_CHECK_ERRORS
}
...
void COGLColorCombiner::InitCombinerCycle12(void)
{
    ...
    if (isNewShader) {
        ...
        if (logLen > 1) {
            ...
            glGetProgramInfoLog(program, logLen, NULL, log);
            if (strlen(log) > 0)
                fprintf(stderr, "RICE: InitCombinerCycle12 log=%s\n", log);
            free(log);
        }
    }
