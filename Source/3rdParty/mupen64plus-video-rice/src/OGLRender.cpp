...
bool OGLRender::RenderTexRect()
{
    ...
    glVertexAttribPointer(VS_COLOR, 4, GL_UNSIGNED_INT_8_8_8_8_REV, GL_TRUE, 0, &colour );
...
bool OGLRender::RenderFillRect(uint32 dwColor, float depth)
{
    ...
    glVertexAttribPointer(VS_COLOR, 4, GL_UNSIGNED_INT_8_8_8_8_REV,GL_FALSE, 0, &colour );
    glVertexAttribPointer(VS_POSITION,4,GL_FLOAT,GL_FALSE,0,&vertices);
...
bool OGLRender::RenderFlushTris()
{
    ...
    { static int rt_cnt = 0; if (++rt_cnt <= 8) {
    ...
    { GLenum e = glGetError(); if (e != GL_NO_ERROR) fprintf(stderr, "RICE: glViewportWrapper error=0x%04X\n", e); }

    { static int vp_cnt = 0; if (++vp_cnt <= 8) {
        GLint vp[4]; glGetIntegerv(GL_VIEWPORT, vp);
        GLboolean scissor_en; glGetBooleanv(GL_SCISSOR_TEST, &scissor_en);
        GLint sc[4]; glGetIntegerv(GL_SCISSOR_BOX, sc);
        fprintf(stderr, "RICE:   vp=(%d,%d,%d,%d) scissor=%d box=(%d,%d,%d,%d) vpLeftW=%d dispH=%d\n",
            vp[0],vp[1],vp[2],vp[3], scissor_en, sc[0],sc[1],sc[2],sc[3],
            windowSetting.vpLeftW, windowSetting.uDisplayHeight);
        for (int vi = 0; vi < 3 && vi < (int)gRSP.numVertices; vi++) {
            fprintf(stderr, "RICE:   vtx[%d] pos=(%.2f,%.2f,%.2f,%.2f)\n", vi,
                g_vtxProjected5[vi][0], g_vtxProjected5[vi][1],
                g_vtxProjected5[vi][2], g_vtxProjected5[vi][3]);
        }
    } }
    ...
    //if options.bOGLVertexClipper == FALSE )
    {
        ...
        { GLenum err = glGetError(); if (err != GL_NO_ERROR) fprintf(stderr, "RICE: glDrawElements error=0x%04X line=%d\n", err, __LINE__); }
    }
    ...
    // Sample pixel at VIEWPORT center AFTER geometry draw (disabled)
/*  else
    {
        //ClipVertexesOpenGL();
        // Redo the index
