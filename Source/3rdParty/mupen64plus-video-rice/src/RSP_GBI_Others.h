...
void RDP_GFX_DumpVtxInfoDKR(uint32 dwAddr, uint32 dwV0, uint32 dwN)
{
        ...
        {
            float x = (float)psSrc[(i + 0) ^ N64_XOR(1)];
            float y = (float)psSrc[(i + 1) ^ N64_XOR(1)];
            float z = (float)psSrc[(i + 2) ^ N64_XOR(1)];

            //uint16 wFlags = CRender::g_pRender->m_dwVecFlags[dwV]; //(uint16)psSrc[3^0x1];

            uint16 wA = psSrc[(i + 3) ^ N64_XOR(1)];
            uint16 wB = psSrc[(i + 4) ^ N64_XOR(1)];
...
void PD_LoadMatrix_0xb4(uint32 addr)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++) 
        {
            int  hi = *(short *)((unsigned char*)data + (((i<<3)+(j<<1)     )^N64_XOR(2)));
            int  lo = *(uint16*)((unsigned char*)data + (((i<<3)+(j<<1) + 32)^N64_XOR(2)));
