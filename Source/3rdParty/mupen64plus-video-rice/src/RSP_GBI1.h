...
void DisplayVertexInfo(uint32 dwAddr, uint32 dwV0, uint32 dwN)
{
        ...
        {
            float x = (float)psSrc[0^N64_XOR(0x1)];
            float y = (float)psSrc[1^N64_XOR(0x1)];
            float z = (float)psSrc[2^N64_XOR(0x1)];

            //uint32 wFlags = g_dwVtxFlags[dwV]; //(uint16)psSrc[3^0x1];
            uint32 wFlags = 0;

            uint8 a = pcSrc[12^N64_XOR(0x3)];
            uint8 b = pcSrc[13^N64_XOR(0x3)];
            uint8 c = pcSrc[14^N64_XOR(0x3)];
            uint8 d = pcSrc[15^N64_XOR(0x3)];
            ...
            float tu = (float)(short)(psSrc[4^N64_XOR(0x1)]);
            float tv = (float)(short)(psSrc[5^N64_XOR(0x1)]);
...
void RSP_MoveMemLight(uint32 dwLight, uint32 dwAddr)
{
    ...
    else
    {
        gRSPn64lights[dwLight].dwRGBA       = pdwBase[0];
        gRSPn64lights[dwLight].dwRGBACopy   = pdwBase[1];
        x       = pcBase[8 ^ N64_XOR(3)];
        y       = pcBase[9 ^ N64_XOR(3)];
        z       = pcBase[10 ^ N64_XOR(3)];
    }
    ...
    LOG_UCODE("       RGBA: 0x%08x, RGBACopy: 0x%08x, x: %d, y: %d, z: %d", 
        ...
        x, y, z);
    { static int li_cnt=0; if (++li_cnt < 8)
        fprintf(stderr, "LIGHT %d: RGBA=0x%08X Copy=0x%08X pos=(%d,%d,%d) range=%.0f\n",
            dwLight, gRSPn64lights[dwLight].dwRGBA,
            gRSPn64lights[dwLight].dwRGBACopy,
            (int)x, (int)y, (int)z, range); }
...
void RSP_MoveMemViewport(uint32 dwAddr)
{
    ...
    scale[0] = *(short *)(g_pRDRAMu8 + ((dwAddr+(0*2))^N64_XOR(2)));
    scale[1] = *(short *)(g_pRDRAMu8 + ((dwAddr+(1*2))^N64_XOR(2)));
    scale[2] = *(short *)(g_pRDRAMu8 + ((dwAddr+(2*2))^N64_XOR(2)));
    scale[3] = *(short *)(g_pRDRAMu8 + ((dwAddr+(3*2))^N64_XOR(2)));

    trans[0] = *(short *)(g_pRDRAMu8 + ((dwAddr+(4*2))^N64_XOR(2)));
    trans[1] = *(short *)(g_pRDRAMu8 + ((dwAddr+(5*2))^N64_XOR(2)));
    trans[2] = *(short *)(g_pRDRAMu8 + ((dwAddr+(6*2))^N64_XOR(2)));
    trans[3] = *(short *)(g_pRDRAMu8 + ((dwAddr+(7*2))^N64_XOR(2)));
