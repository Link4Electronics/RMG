...
#if defined(__BIG_ENDIAN__) || (defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__)
// On BE: *(uint32*)RDRAM reads 0xRRGGBBAA (N64 R8G8B8A8 in native byte order).
// Need to rotate-right by 8 to get A8R8G8B8 = 0xAARRGGBB.
#define RGBA_TO_ARGB(rgba) ((((rgba) & 0xFF) << 24) | ((rgba) >> 8))
#else
// On LE: *(uint32*)RDRAM reads 0xAABBGGRR (bytes reversed by LE load).
// Macro rearranges to 0xAARRGGBB.
