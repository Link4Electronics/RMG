...
TxtrCacheEntry* LoadTexture(uint32 tileno)
{
    ...
    { static int td_cnt = 0; if (++td_cnt <= 100) {
        fprintf(stderr,
            ...
            tile.dwMaskS, tile.dwMaskT, tile.dwShiftS, tile.dwShiftT,
        ...
        for (int ri = 0; ri < 16; ri++)
            fprintf(stderr, " %02X", raw[ri]);
        fprintf(stderr, "\n");
        // dump raw palette data first 32 bytes
        uint8 *praw = (uint8*)(g_wRDPTlut);
        fprintf(stderr, "  PAL[0..31]:");
...
{
{ static int tl_cnt = 0; if (++tl_cnt <= 100) fprintf(stderr,
    "LOADTLUT: w0=0x%08X w1=0x%08X bUseFullTMEM=%u\n",
    gfx->words.w0, gfx->words.w1, options.bUseFullTMEM); }
...
void DLParser_LoadBlock(Gfx *gfx)
{
    ...
    // DIAG: check if loading to TLUT region (0x100 = palette bank 0)
    if (tile.dwTMem >= 0x100 || tile.dwFormat == 0 /*G_IM_FMT_CI*/)
    {
        static uint32 diagCount = 0;
        if (++diagCount <= 20)
            fprintf(stderr, "LOADBLOCK: tileno=%u dwTMem=0x%X fmt=%u sz=%u pal=%u size=%u bFullTMEM=%u\n",
                tileno, tile.dwTMem, tile.dwFormat, tile.dwSize, tile.dwPalette, size, options.bUseFullTMEM);
    }
...
void DLParser_LoadTile(Gfx *gfx)
{
    ...
    // DIAG: check if loading to TLUT region or CI format
    if (tile.dwTMem >= 0x100 || tile.dwFormat == 0 /*G_IM_FMT_CI*/)
    {
        static uint32 diagCount = 0;
        if (++diagCount <= 20)
            fprintf(stderr, "LOADTILE:  tileno=%u dwTMem=0x%X fmt=%u sz=%u pal=%u bFullTMEM=%u\n",
                tileno, tile.dwTMem, tile.dwFormat, tile.dwSize, tile.dwPalette, options.bUseFullTMEM);
    }

    // Populate g_wRDPTlut[] from RDRAM when loading palette data to TLUT region
    if (tile.dwTMem >= 0x100 && tile.dwFormat == 0 /*G_IM_FMT_CI*/)
    {
        uint32 totalBytes = bpl * height;
        uint32 address = g_TI.dwAddr + tile.tl * g_TI.bpl + (tile.sl << g_TI.dwSize >> 1);
        if (totalBytes > 0 && (address + totalBytes) <= g_dwRamSize)
        {
            uint16 *palSrc = (uint16*)(g_pRDRAMu8 + address);
            uint32 dwTMEMOffset = tile.dwTMem - 256;
            uint32 palCount = totalBytes / 2;
            if (palCount + dwTMEMOffset > 512) palCount = 512 - dwTMEMOffset;
            if (palCount > 0)
            {
                fprintf(stderr, "  PALFIX: loading %u palette entries from RDRAM 0x%X to g_wRDPTlut[%u..%u]\n",
                    palCount, address, dwTMEMOffset, dwTMEMOffset + palCount - 1);
                for (uint32 i = 0; i < palCount; i++)
                    g_wRDPTlut[(i + dwTMEMOffset) ^ N64_XOR(1)] = palSrc[i ^ N64_XOR(1)];
            }
        }
    }
}
...
void DLParser_TexRect(Gfx *gfx)
{
    ...
    { static int tr_cnt=0; if (++tr_cnt < 8) fprintf(stderr,
        "TEXR: dwPC=%08X w0=%08X w1=%08X h1=%08X c2=%08X h2=%08X c3=%08X "
        "XH=%u YH=%u XL=%u YL=%u S=0x%04X T=0x%04X DSDX=0x%04X DTDY=0x%04X "
        "fS0=%.2f fT0=%.2f\n",
        dwPC, gfx->words.w0, gfx->words.w1, dwHalf1, dwCmd2, dwHalf2, dwCmd3,
        dwXH, dwYH, dwXL, dwYL, uS, uT, uDSDX, uDTDY,
        (float)(short)uS/32.0f, (float)(short)uT/32.0f); }
