...
void InitVertex(uint32 dwV, uint32 vtxIndex, bool bTexture)
{
    ...
    { static int vz_cnt=0; if (++vz_cnt <= 9)
        fprintf(stderr, "ZDBG: dwV=%u vtxIdx=%u z=%.3f zw=%.3f z_ogl=%.3f\n",
            dwV, vtxIndex, g_vecProjected[dwV].z, g_vtxTransformed[dwV].w,
            (g_vecProjected[dwV].z + 1.0f) * 0.5f); }
...
inline void ReplaceAlphaWithFogFactor(int i)
{
    ...
    {
        // Use fog factor to replace vertex alpha (endian-safe: alpha always in MSB of COLOR_RGBA)
        if( g_vecProjected[i].z > 1 )
            g_dwVtxDifColor[i] = (g_dwVtxDifColor[i] & 0x00FFFFFF) | (0xFF << 24);
        if( g_vecProjected[i].z < 0 )
            g_dwVtxDifColor[i] = (g_dwVtxDifColor[i] & 0x00FFFFFF);
        else
            g_dwVtxDifColor[i] = (g_dwVtxDifColor[i] & 0x00FFFFFF) | (((uint8)(g_vecProjected[i].z*255)) << 24);
    }
}
...
void ProcessVertexDataSSE(uint32 dwAddr, uint32 dwV0, uint32 dwNum)
{
    ...
    for (uint32 i = dwV0; i < dwV0 + dwNum; i++)
    {
        ...
        if( gRSP.bLightingEnable )
        {
            ...
            g_dwVtxDifColor[i] = (g_dwVtxDifColor[i] & 0x00FFFFFF) | (vert.rgba.a << 24); // still use alpha from the vertex
        }
        else
        {
...
void ProcessVertexDataNoSSE(uint32 dwAddr, uint32 dwV0, uint32 dwNum)
{
    ...
    { static int md2=0; if (++md2<=2) {
        Matrix &p = gRSP.projectionMtxs[gRSP.projectionMtxTop];
        fprintf(stderr, "PROJMTX: %.6f %.6f %.6f %.6f  %.6f %.6f %.6f %.6f  %.6f %.6f %.6f %.6f  %.6f %.6f %.6f %.6f\n",
        ...
        fprintf(stderr, "MODVMTX: %.6f %.6f %.6f %.6f  %.6f %.6f %.6f %.6f  %.6f %.6f %.6f %.6f  %.6f %.6f %.6f %.6f\n",
            gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[0][0],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[0][1],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[0][2],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[0][3],
            gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[1][0],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[1][1],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[1][2],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[1][3],
            gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[2][0],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[2][1],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[2][2],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[2][3],
            gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[3][0],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[3][1],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[3][2],gRSP.modelviewMtxs[gRSP.modelViewMtxTop].m[3][3]);
    } }
...
void ProcessVertexDataDKR(uint32 dwAddr, uint32 dwV0, uint32 dwNum)
{
    ...
    for (uint32 i = dwV0; i < end; i++)
    {
        ...
        g_vtxNonTransformed[i].x = (float)*(short*)((pVtxBase+nOff + 0) ^ N64_XOR(2));
        g_vtxNonTransformed[i].y = (float)*(short*)((pVtxBase+nOff + 2) ^ N64_XOR(2));
        g_vtxNonTransformed[i].z = (float)*(short*)((pVtxBase+nOff + 4) ^ N64_XOR(2));
        ...
        short wA = *(short*)((pVtxBase+nOff + 6) ^ N64_XOR(2));
        short wB = *(short*)((pVtxBase+nOff + 8) ^ N64_XOR(2));
...
void ProcessVertexDataPD(uint32 dwAddr, uint32 dwV0, uint32 dwNum)
{
    ...
    for (uint32 i = dwV0; i < dwV0 + dwNum; i++)
    {
        ...
        if( gRSP.bLightingEnable )
        {
            ...
            g_dwVtxDifColor[i] = (g_dwVtxDifColor[i] & 0x00FFFFFF) | ((uint8)a << 24);    // still use alpha from the vertex
        }
        else
        {
            if( (gRDP.geometryMode & G_SHADE) == 0 && gRSP.ucode < 5 )  //Shade is disabled
...
void ProcessVertexData_Rogue_Squadron(uint32 dwXYZAddr, uint32 dwColorAddr, uint32 dwXYZCmd, uint32 dwColorCmd)
{
    ...
    for (i = dwV0; i < dwV0 + dwNum; i++)
    {
        ...
        if( gRSP.bLightingEnable )
        {
            ...
            g_dwVtxDifColor[i] = (g_dwVtxDifColor[i] & 0x00FFFFFF) | (vertcolors.a << 24);    // still use alpha from the vertex
        }
        else
        {
            if( (gRDP.geometryMode & G_SHADE) == 0 && gRSP.ucode < 5 )  //Shade is disabled
